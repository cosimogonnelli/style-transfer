...
def minimize_with_lbfgs(sess, net, optimizer, init_img):
  ...
  while block < args.blocks:
    ...
    output_img = sess.run(net['input'])
    out_dir = os.path.join(args.img_output_dir, args.img_name, 'block')
    maybe_make_directory(out_dir)
    img_path = os.path.join(out_dir, args.img_name+str(block)+'.png')
  ...
  while block < args.blocks:
    while (iterations < args.max_iterations):
      ...
      if iterations % args.print_iterations == 0 and args.verbose:
        ...
        output_img = sess.run(net['input'])
        out_dir = os.path.join(args.img_output_dir, args.img_name, 'iter')
        maybe_make_directory(out_dir)
        img_path = os.path.join(out_dir, args.img_name+str(iterations)+'.png')
        write_image(img_path, output_img)
      iterations += 1
    block += 1
